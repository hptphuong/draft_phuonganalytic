(function() {
    ...
    $fsaCore.prototype.requestImage = function(m_tracker) {
        // var m_tracker = this;
    ...
    fsaCore.create('send', function(a) {
        ...
        // check tracker is exist or not
        a.split(".").length < 2 ? m_name = "t0" : m_name = a.split(".").slice(0, -1).join(".");
        if (!fsaCore.getTrackerByName(m_name))
            return
        else
            m_tracker = fsaCore.getTrackerByName(m_name);
        args = Array.prototype.slice.call(arguments);
        args.unshift(m_tracker);
        fsaCore.requestImage.apply(this, args);
        // m_tracker=
        console.log("loaded send");
    });
